...
function createNoteFromInput(e){
    ...
    while(window.localStorage.getItem(noteTitle)){
        if(Number.isInteger(parseInt(noteTitle.charAt(noteTitle.length -1)))){
            noteTitle = noteTitle.replace(noteTitle.charAt(noteTitle.length -1), parseInt(noteTitle.charAt(noteTitle.length -1)) +1);     
        }
        else{
            noteTitle += " - 1";
        }
...
function removeItem(e){
    ...
    if(e.target.classList.contains("delete")){
        if(confirm("Are you sure that this is to be deleted? ")){
